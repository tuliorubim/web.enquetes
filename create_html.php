...
class Create_HTML extends DesignFunctions {
	...
	public function form_conteudo2 ($inds) {
		...
		if ($select[5] && $content_type == 1) {
			$sql = "select ci.imagem, '<button id=\"'+ci.idImagem+'\">Reutilizar no texto</button>' from content_image ci inner join cliente c on ci.cd_usuario = c.idCliente where c.idCliente = $idu";
			$sizes = "width='300'";
