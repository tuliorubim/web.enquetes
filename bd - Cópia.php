...
if ($idu === 0 || ($idu !== 0 && $user == NULL)) {
	$redirect = "criar_enquete.php";
	if (strpos($_SERVER['REQUEST_URI'], 'enquete.php') !== FALSE) $redirect = $_SERVER['REQUEST_URI'];
	login($POST, 'cliente', $redirect, false, array(), array('usuario', 'senha', 'enter'));
}
	...
	if (!$logged_in) {
		...
		if ($idCliente == NULL) {
			...
			if ($usuario != $email) {
				...
				echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=criar_enquete.php?ide=".$ret2[1]."&idc=".$POST["cd_categoria"]."'>";
			} else {
			...
			$redirect = "criar_enquete.php?ide=".$ret[1]."&idc=".$POST["cd_categoria"];
			login($POST, 'cliente', $redirect, false, array(), array('usuario', 'senha', 'button'));
		}
	} else {
		$POST['cd_usuario'] = $_SESSION[$idSession]; 
		$ret = save($tabela, $variaveis, $tipos, $POST, array(), -1);
		echo "<meta HTTP-EQUIV='Refresh' CONTENT='0;URL=criar_enquete.php?ide=".$ret[1]."&idc=".$POST["cd_categoria"]."'>";
	}
